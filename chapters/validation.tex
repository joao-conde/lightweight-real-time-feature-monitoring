...
In the batch phase, we used 100 Spark executors, each with 12G of memory. We determined that one week's worth of data for this merchant was on average equivalent to 37,803 events. We set the half-life to be $n_{1/2}=37,803$ and discard events whose contribution is 6\% or less, which is equivalent to using windows of data of four times the half-life (as detailed in Section \ref{sec:ema-hist}). Hence, our samples' windows had 151,212 events. Using Equation \ref{eq:optimal-n-samples}, we determined that for 183 features, a FWER of 1\% and $\gamma$ (as defined in Section \ref{sec:nsamples}) of 10\% we needed to make a minimum of 42,137 samples. We conclude the batch analysis in 5 hours and 34 minutes.

After the batch phase, we had the necessary inputs for the streaming phase (detailed in Section \ref{sec:batch-artifacts-summary}), namely, for each feature, the reference histograms, the list of observed divergence values and the last sample's histogram. We then used the target dataset, simulating a stream of ordered events. Our monitoring system processed event by event and reported divergent features and associated p-values. We performed our multiple hypotheses with the Holm-Bonferroni correction (Section \ref{sec:holmbonferroni}) for every 1000 events. It took us 382,363 milliseconds to process the 4,032,505 transactions, amounting to a throughput of 10,546 transactions per second (TPS).

Our system produced a large list of alerts. We had alerts from nearly the beginning of the target period until the end. To try and make sense of these alerts, we questioned Feedzai's data scientists that worked directly with these data sources. We found out that this merchant and in particular these datasets were known to suffer a lot from concept drift. Additionally, the data scientists performed some data preprocessing and cleaning, removing some of the missing values, but only on the reference dataset, which further augmented the differences between the target and reference datasets. These seemed like plausible reasons for the large number of alarms generated.

%x1 = completion_rate_per_card_72h
We set out to validate this hypothesis. Due to the large number of features (183) we only show the plots and visualizations drawn for one. Furthermore, we refer to features with anonymized identifiers, like our example feature, x1. Figure \ref{fig:merchant-x1-corrected-pvalues} shows the corrected p-values (Formula \ref{for:corrected-pvalue}) for feature x1 at for each event of the time-series and Figure \ref{fig:merchant-x1-corrected-pvalues-zoom} provides a closer look on the region where the p-values drop below the threshold (FWER = 1\%).
\begin{center}
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[width=1\linewidth]{figures/merchant-x1-corrected-pvalues.png}
  \captionof{figure}{x1 corrected p-values and threshold}
  \label{fig:merchant-x1-corrected-pvalues}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[width=1\linewidth]{figures/merchant-x1-corrected-pvalues-zoom.png}
  \captionof{figure}{x1 zoomed in corrected p-values and threshold}
  \label{fig:merchant-x1-corrected-pvalues-zoom}
\end{minipage}
\end{center}
We see that this feature drops below the alarm threshold and never rises above it, effectively being considered divergent for the rest of the target period. Furthermore, it reaches p-values very close to zero and eventually becomes zero, throughout the remaining target period. Is such divergence possible? To find out, let us plot the reference and target time-series for feature x1. Figure \ref{fig:merchant-x1-reference} shows the reference time-series plot and Figure \ref{fig:merchant-x1-target} shows the target time-series for feature x1.
\begin{center}
\begin{minipage}{.5\textwidth}
  \centering
...
\end{minipage}
\end{center}
It became clear to us why this feature and many others were being alerted from the start of the dataset until the end. The domain of values is significantly different. The reference period was cleaned of missing values, usually represented by -1. Hence, our reference was built expecting values from 0 to 1 with a certain distribution and in the stream analysis, we found a large volume of missing values or -1. This explains the high number of alerts.
...
\subsection{Similar reference and target periods}
In this next experiment, we set out to test our system in a low drift scenario,... 

\textcolor{red}{SPLIT TARGET PERIOD IN TWO, USING SAME PERIOD GUARANTEES SIMILAR PERIODS? BUT MAYBE IT DOES NOT (LOOK AT x1 TARGET TIMESERIES, 1/3 has no MISSING VALUES AS WELL. STILL VERY DIFFERENT FROM 2/3)}.
